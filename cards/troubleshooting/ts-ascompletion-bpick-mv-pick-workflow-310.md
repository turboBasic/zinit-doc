...
With `as"completion" from"gh-r" bpick"completions_zsh"`, the completion file is downloaded but not linked into zinit's completions directory. Running `zinit csearch` or trying to use the completion shows nothing. Tab completion for the tool does not work.

## Cause

`as"completion"` triggers zinit's completion management only when `pick` points to a file starting with `_`. The downloaded file name (`completions_zsh`) does not start with `_`, so zinit does not recognize it as a completion file. The `mv` ice must rename it, and `pick` must reference the renamed file. The `mv` ice runs after extraction, so renaming to the `_` prefix form happens before zinit tries to install it.
...
```zsh
# With id-as to avoid conflicts when downloading completion separately from the binary
zi ice from"gh-r" as"completion" \
...
The `as"completion"` ice requires the completion file to either be named with a leading underscore (`_foo`) or to be explicitly selected via `pick`. Without both `mv` and `pick`, zinit silently skips completion installation with no error message.
